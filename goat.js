...
const scoreDisplay = document.getElementById('score'); // Corrected to getElementById
let score = 0;
...
document.addEventListener('keydown', (event) => {
    if (event.key === 'ArrowLeft' && goatPosition > 0) {
        goatPosition -= 20;
    } else if (event.key === 'ArrowRight' && goatPosition < 250) { // Fixed typo: goatposition to goatPosition
        goatPosition += 20;
    }
...
function moveObject(object, type) {
    let objectTop = parseInt(window.getComputedStyle(object).getPropertyValue('top')); // Added missing parameter 'top'
    if (objectTop >= 600) {
...
function checkCollision() {
    const goatRect = goat.getBoundingClientRect(); // Added to get goat's rectangle
    const leafRect = leaf.getBoundingClientRect(); // Added to get leaf's rectangle
    const bikeRect = bike.getBoundingClientRect(); // Added to get bike's rectangle
